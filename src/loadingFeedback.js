...
export function normalizeLayerLoading(layer = {}) {
  ...
  return {
    id: String(layer.id || ''),
    label: String(layer.name || layer.id || t('loading.layerFallback')),
    loading,
    disabling,
...
export function reduceTrafficSyncFeedback(previous, {
  ...
  if (busy) {
    return {
      ...
      // keyless build.
      label: label || t('loading.trafficSync'),
...
export function presentLoadingFeedback(state, summary, nowMs) {
  ...
  const label = summary.disabling
    ? t('loading.turningOffLiveData')
    : summary.refresh ? t('loading.refreshingLiveData') : t('loading.loadingLiveData');
