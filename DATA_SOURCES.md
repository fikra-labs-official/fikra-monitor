...
- **Attribution is shown in-app** and listed here. Keep it intact. Required Google attribution renders on the on-globe credit line (bottom-left, `#cesium-credits`); the Cesium ion logo appears there only when ion content is configured. Every per-layer credit below is registered into the expandable **"Data attribution"** lightbox on that line (`src/data/dataCredits.js` → `viewer.creditDisplay.addStaticCredit`). The applicable credits stay visible in clean-view and recording modes.
...
| **OpenStreetMap (Nominatim)** | Reverse-geocoded cockpit place labels and cached administrative boundary polygons | ODbL 1.0 + Nominatim usage policy | "© OpenStreetMap contributors" |
